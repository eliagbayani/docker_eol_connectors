...
[ -d ${TARGET_PATH}/opendata ]          && echo "Symlink already exists (opendata)" || 
    ln -s /opt/homebrew/var/www/eol_php_code/applications/opendata/ opendata 
[ -d ${TARGET_PATH}/opendata_uploads ]  && echo "Symlink already exists (opendata_uploads)" || 
...
[ -d ${TARGET_PATH}/other_files2 ]              && echo "Symlink already exists (other_files2)" || 
    ln -s /Volumes/Crucial_2TB/other_files2/ other_files2
[ -d ${TARGET_PATH}/cache_LiteratureEditor ]    && echo "Symlink already exists (cache_LiteratureEditor)" || 
    ln -s /Volumes/Crucial_2TB/cache_LiteratureEditor/ cache_LiteratureEditor
